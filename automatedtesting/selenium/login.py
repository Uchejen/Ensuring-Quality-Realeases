...
def add_items(driver):
    ...
    items_count = driver.find_elements(By.CLASS_NAME, "shopping_cart_badge").text
...
def remove_items(driver):
    ...
    cart_list = driver.find_elemets(By.CLASS_NAME, "cart_item")
    for item in cart_list:
        item_text = item.find_element(By.CLASS_NAME, 'inventory_item_name').text
        print(f"\t- {item_text} was removed.")
        item.find_element(By.CLASS_NAME, 'cart_button').click()
    items_count = len(driver.find_elemets(By.CLASS_NAME, "cart_item"))
    assert int(items_count) == 0
    print("All item were removed successfully.")

    driver.find_elemets(By.CLASS_NAME, "shopping_cart_link").click()
